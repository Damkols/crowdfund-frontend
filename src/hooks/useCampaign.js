...
const useCampaign = (id) => {
 ...
 useEffect(() => {
  const fetchCampaign = async () => {
   ...
   if (!campaignId || campaignId > campaignLength) return setState("NOT_FOUND");
   try {
    const contract = await getCrowdfundContract(provider, false);

    const campaignStruct = await contract.crowd(campaignId);
    const contribute = await contract.getContributors(campaignId);
    const contributorsArray = contribute.toArray();
