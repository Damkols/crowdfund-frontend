...
const Campaign = ({ campaign }) => {
 return (
  <div className="bg-white w-[35%] sm:max-w-sm border-2 border-blue-200 shadow-lg rounded-xl overflow-hidden py-8">
   <Link to={`/campaign/${campaign.id}`}>
    <div className="px-6 py-4">
     ...
     <p className="mt-4 font-bold text-gray-700 text-sm">
      Contributors -{" "}
      {campaign?.contributors.length > 0
       ? campaign?.contributors.map((c) => <p>{c}</p>)
       : "No Contributors yet"}
     </p>
    </div>
    <div className="flex flex-col gap-3 px-6 pt-2 pb-2">
     ...
     <span
      className={`${
       campaign?.isActive ? "bg-green-400 text-white" : "bg-red-400 text-white"
